...
import { A4Dimensions, DefaultMargins, getOuterHeightOfElement } from '../shared/utils'
...
export const pagesCounter = (): number => {
  return allPages.reduce(
    (accumulator, page) => accumulator + (page.firstElementChild.childNodes.length ? 1 : 0),
    0
  );
};
...
const distributeNodesOnPages = (nodes: ChildNode[]): void => {
  ...
  let currentPage = 1;
  const marginTopAndBottom = (heightPoint * currentMargins.bottom) +  (heightPoint * currentMargins.top)
...
const buildPage = (pageNumber: number, nodesToInsertInPage: ChildNode[], lastPage = false): void => {
